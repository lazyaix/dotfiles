...
local GLYPH_SEARCH = nf.md_telescope --[[ 󰭎 ]]
...
-- AstroDark tab title colors
---@type table<string, Cells.SegmentColors>
-- stylua: ignore
local colors = {
  text_default          = { bg = '#3A3E47', fg = '#ADB0BB' },
  text_hover            = { bg = '#23272F', fg = '#ADB0BB' },
  text_active           = { bg = '#50A4E9', fg = '#1A1D23' },

  unseen_output_default = { bg = '#3A3E47', fg = '#EB8332' },
  unseen_output_hover   = { bg = '#23272F', fg = '#EB8332' },
  unseen_output_active  = { bg = '#50A4E9', fg = '#EB8332' },

  scircle_default       = { bg = 'rgba(0, 0, 0, 0.4)', fg = '#3A3E47' },
  scircle_hover         = { bg = 'rgba(0, 0, 0, 0.4)', fg = '#23272F' },
  scircle_active        = { bg = 'rgba(0, 0, 0, 0.4)', fg = '#50A4E9' },
}
...
---@param panes any[] WezTerm pane objects
...
function Tab:set_info(event_opts, tab, max_width)
  ...
  -- Add tab index to the title (1-based)
  local tab_index = tab.tab_index + 1
  local index_prefix = string.format('%d: ', tab_index)
  inset = inset + index_prefix:len()

  if self.title_locked then
    self.title = index_prefix .. create_title('', self.locked_title, max_width, inset)
    return
  end
  self.title = index_prefix .. create_title(process_name, tab.active_pane.title, max_width, inset)
end
...
M.setup = function(opts)
  ...
  -- CUSTOM EVENT: manually update tab name (locked until reset)
  ...
  -- CUSTOM EVENT: unlock manually set tab name
  ...
  -- BUILTIN EVENT: format tab title
